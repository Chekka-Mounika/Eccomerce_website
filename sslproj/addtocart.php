...
$productid=$row['product_id'];
  ...
  if(!isset($_SESSION['loggedin'])){
    echo "<div class='notlogaw'><div><b>You are not logged in<div>Login to add products to cart</b></div></div></div>";
 }
...
$userid=$_SESSION["userid"];
    $sql3="SELECT * FROM `cart`  WHERE `product_id`=$productid AND `user_id`='$userid'";
    $result2=mysqli_query($conn,$sql3);
    // $row2=mysqli_fetch_assoc($result2);

    if(mysqli_num_rows($result2)>0){
        echo"<div class='yeslogaw'><div>Product already there in cart</div>";
        echo"<div style='margin-top:20px;'><a href='cart.php'><button style='margin-right:20px;padding:10px;border-radius:10px;  background-color:  #c80c54;color:white; cursor:pointer;'><b>View cart</b></button></a>";
    ...
    $sql2="INSERT into `cart`(`product_id`, `user_id`,`product_name`,`product_price`) VALUES ('$productid','$userid','$productname','$productprice')";
    $result=mysqli_query($conn,$sql2);
    echo "<div class='yeslogaw'><div>Product added to cart succesfully</div>";
    echo"<div style='margin-top:20px;'><a href='cart.php'><button style='margin-right:20px;padding:10px;border-radius:10px;  background-color:  #c80c54;color:white; cursor:pointer;'><b>View cart</b></button></a>";
    echo '<a href="single.php?productid='.$productid.'"><button style="margin-right:20px;padding:10px;border-radius:10px; background-color:  #c80c54;color:white;cursor:pointer"><b>Continue shopping</b></button></a></div></div>';
    }
