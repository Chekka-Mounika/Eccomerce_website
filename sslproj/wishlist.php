...
<?php
if(!isset($_SESSION['loggedin'])){
    echo "<div class='notlogaw'style='color:white;'><div><b>You are not logged in<div>Login to view your wishlist products</b></div></div></div>";
 }
 if(isset($_SESSION['loggedin']) && $_SESSION['loggedin']==true){
    $userid=$_SESSION["userid"];
    $sql1="SELECT * FROM `wishlist`  WHERE `user_id`='$userid'";
    $result=mysqli_query($conn,$sql1);
    if(mysqli_num_rows($result)==0)
    {
        echo"<div style='color:white;'>You don't have any products in your wishlist</div>";
    }
    else{
        echo "<table style='color:white;'><tr><th>Product name</th><th>Product price</th></tr>";
